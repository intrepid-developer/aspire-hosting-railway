...
- Apply no longer `serviceCreate`s an image-less Railway service per bucket to host `${{uploads.ENDPOINT}}` (and related) variables. That leftover Offline compute card confused people. `AddRailwayBucketClient` reads `ConnectionStrings:{name}` only; apply writes the resolved connection string onto `WithReference` compute services at deploy time. The plan keeps a `railway-bucket://{name}` placeholder — never S3 keys, never `${{bucket.ENDPOINT}}`. Existing Offline leftovers from earlier previews can be deleted in the Railway dashboard; v1 does not adopt or `serviceDelete` them (and still has no `bucketDelete`).
...
- Package version now tracks 13.5.1. This release is `13.5.1-preview.1`.
- This is a pin bump, not a Railway API change. `Microsoft.Extensions.*` stays on 10.0.11.
