...
/// <summary>
/// Resolves Railway bucket S3 endpoint and addressing style.
/// Documented host is <c>t3.storageapi.dev</c> (virtual-hosted).
/// <c>storage.railway.app</c> remains a legacy alias for the same style.
/// Prefer <c>bucketS3Credentials.urlStyle</c> when Railway returns it.
/// </summary>
internal static class RailwayBucketS3Addressing
{
    ...
    internal static string FormatConnectionString(
        string endpoint,
        string accessKeyId,
    ...
    {
        var value =
            $"Endpoint={endpoint};AccessKeyId={accessKeyId};SecretAccessKey={secretAccessKey};Bucket={bucketName};Region={region};ForcePathStyle={(forcePathStyle ? "true" : "false")}";
        if (!string.IsNullOrWhiteSpace(urlStyle))
        {
            value += $";UrlStyle={urlStyle.Trim()}";
        }
