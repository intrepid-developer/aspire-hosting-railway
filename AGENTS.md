...
- `IDeploymentStateManager` ids must be flatten-safe objects (Aspire's file state manager does not round-trip JSON arrays). Persist created-vs-adopted flags in that same store (`CreatedProject`, `CreatedEnvironments`, `CreatedServices`, `CreatedCustomDomains`, `CreatedServiceDomains`). Official managed-template region ids last applied are also flatten-safe objects (`ManagedRegions`). Tokens, bucket secrets, custom-domain verification tokens, and backup payloads stay out of plan files and state. TCP proxies out of scope.
...
| `PublishAsRailwayPostgres` / `Redis` `Region` | `RailwayRegion?`. Plan `region` as official compute ids. After template service id exists, at most one `serviceInstanceUpdate` with `region` + `numReplicas` 1. Volume backup uses `volumeInstanceBackupScheduleUpdate` and does not add a second region update. Subsequent applies skip a region-only re-send when that region is already recorded; include region on a later `serviceInstanceUpdate` so omitting it cannot reset US. Do not invent a `templateDeployV2` region field. |
