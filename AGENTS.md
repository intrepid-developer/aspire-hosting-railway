...
| Area | Operations |
| --- | --- |
| Project / env | `project`, `projectCreate`, `environmentCreate`, `environment`, `environmentPatchCommitStaged`, `regions` |
| Services | `serviceCreate`, `serviceInstanceUpdate`, `serviceInstanceLimitsUpdate`, `serviceInstanceDeployV2`, `variableCollectionUpsert`, `serviceDomainCreate` |
| Domains | `domains`, `customDomain`, `customDomainAvailable`, `customDomainCreate`, `customDomainUpdate` |
...
Never invent mutation or query names. Never `pluginCreate`. Never `cronCreate` / `scheduleCreate`. Never `customDomainDelete` / `customDomainIssueCertificate` on this path. Never `volumeInstanceBackupCreate` / `Delete` / `Lock` / `Restore`, `volumeInstancePITRRestore`, `enablePitrForHaCluster` / `disablePitrForHaCluster`. Do not invent `WAL_ARCHIVE_*` or `bucketCreate` of `Postgres-PITR`. PITR enable is HA-only; #30 stays open. #22 destroy must not invent project, environment, backup, or volume deletes.
...
- `IDeploymentStateManager` ids must be flatten-safe objects (Aspire's file state manager does not round-trip JSON arrays). Tokens, bucket secrets, custom-domain verification tokens, and backup payloads stay out of plan files and state. TCP proxies out of scope. Domain destroy is a later slice.
