...
| `PublishAsRailwayPostgres` / `Redis` `Region` | `RailwayRegion?`. Plan `region` as official compute ids. After template service id exists, `serviceInstanceUpdate` with `region` + `numReplicas` 1. Re-send on later updates (volume backups). Do not invent a `templateDeployV2` region field. |
