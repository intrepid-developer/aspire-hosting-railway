...
| Buckets | `bucketCreate`, `environmentStageChanges`, `environmentPatchCommit`, `bucketS3Credentials` |
...
- Official `Region.region` ids only via `RailwayRegion` (`UsWest2` / `UsEast4` / `EuropeWest4` / `AsiaSoutheast1`). Reject airport codes and older ids on deserialized plans. Cap total replicas at 50. Bucket `RailwayBucketRegion` is the Tigris set (`iad` / `sjc` / `ams` / `sin`); reject compute ids on buckets and Tigris codes on Postgres / Redis.
...
- Adopt buckets by name from `project.buckets`. Never pass a service id to `bucketS3Credentials`. After `bucketCreate`, stage + commit `EnvironmentConfig.buckets.{id}` (`region` Tigris airport code, default `iad`; `isCreated: true`) via `environmentStageChanges` then `environmentPatchCommit`. Then retry `bucketS3Credentials` until keys exist. Do not send compute-region strings. After a compute service id exists, the same mutations set `EnvironmentConfig.services.{serviceId}.deploy.registryCredentials` (`username` + `password` only) so Railway can pull private GHCR images. Do not add registry fields onto `serviceInstanceUpdate`. Fail if the image host is private and no credentials resolve. Tokens stay out of plan and state. Match `environment.volumeInstances` by Postgres `serviceId`; retry if not visible; fail if none matches. Do not use `adminVolumeInstancesForVolume` or `volumeInstance(id)` unless the id is known. Service has no `volumes` field.
...
| Generated `*.up.railway.app` | `WithExternalHttpEndpoints()` only. List `domains` first; adopt if `serviceDomains` has an id; also skip create when `CreatedServiceDomainIds` already has the service. Create only when the list is empty. Deploy does not delete extras; leftover hosts from earlier previews can be removed in the Railway dashboard (Networking). `serviceDomainDelete` is destroy-only. |
...
| `RailwayBucketRegion` / `AddRailwayBucket` `WithRegion` | Tigris codes `iad` / `sjc` / `ams` / `sin` only. Unset defaults to `iad`. Plan `region`. After `bucketCreate`, `CreateInstancePatch(bucketId, region)` via `environmentStageChanges` + `environmentPatchCommit`. Adopt-by-name does not re-patch. Immutable after provision. |
| `PublishAsRailwayPostgres` / `Redis` `Region` | `RailwayRegion?`. Plan `region` as official compute ids. After first-time template create, at most one `serviceInstanceUpdate` with `region` + `numReplicas` 1. Do not call the region update twice in one `ApplyAsync`. Volume backups are `volumeInstanceBackupScheduleUpdate` only. Subsequent applies do not re-send region as its own update; re-include only when already sending a `serviceInstanceUpdate`. Do not invent a `templateDeployV2` region field. |
