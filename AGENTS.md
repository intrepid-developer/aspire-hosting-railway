...
- Adopt buckets by name from `project.buckets`. Never pass a service id to `bucketS3Credentials`. After `bucketCreate`, stage + commit `EnvironmentConfig.buckets.{id}` (`region` Tigris airport code, default `iad`; `isCreated: true`) via `environmentStageChanges` then `environmentPatchCommit`. Then retry `bucketS3Credentials` until keys exist. Do not send compute-region strings. Match `environment.volumeInstances` by Postgres `serviceId`; retry if not visible; fail if none matches. Do not use `adminVolumeInstancesForVolume` or `volumeInstance(id)` unless the id is known. Service has no `volumes` field.
