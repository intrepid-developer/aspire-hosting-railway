...
| `serviceInstanceUpdate` | Always pass `environmentId`. Fields used: `source.image`, `multiRegionConfig`, `sleepApplication`, `numReplicas` (only when `WithReplicas` is set alone — never with `multiRegionConfig`), `healthcheckPath`, `healthcheckTimeout`, `restartPolicyType`, `restartPolicyMaxRetries`, `startCommand`, `preDeployCommand`, `overlapSeconds`, `drainingSeconds`, `cronSchedule`. Official Postgres / Redis templates send `region` plus `numReplicas` 1 when a compute region is set; they do not send `multiRegionConfig`. Returns Boolean — settings only, not a canvas deploy. A later update that omits `region` can reset the template to US West — re-include region only when already sending this mutation. Do not send a second standalone region update in the same apply (volume backups use `volumeInstanceBackupScheduleUpdate`). Subsequent applies do not re-send region as its own update. Omit unset fields; do not send `null`. Do not add `vCPUs` / `memoryGB` or `registryCredentials` here. No GraphQL field named `serverless`. `ServiceInstance` has no `multiRegionConfig` read field. |
...
| `templateDeployV2` | Deploys that fetched template. Confirmed input is `projectId`, `environmentId`, `templateId`, `serializedConfig`. There is no confirmed region field — do not invent one. |
...
| `bucketS3Credentials` | Reads S3 credentials after the instance exists. `projectId` required. Select `bucketName` (not `bucket`) and `urlStyle` (`virtual` / `path`). Prefer returned `endpoint` (documented host is `https://t3.storageapi.dev`; `storage.railway.app` is a legacy alias). Never pass a service id. Do not persist the secret. |
...
- Do not send scale / limits / healthcheck / restart / start / pre-deploy / teardown / cron / custom domains for `PublishAsRailwayPostgres` / `PublishAsRailwayRedis` / buckets. The template-region exception is `serviceInstanceUpdate.region` plus `numReplicas` 1 (not `multiRegionConfig`).
- Do not use `environmentPatchCommit` / staged patches for those compute settings.
- Bucket **instance** provision is the exception: after `bucketCreate`, apply stages then commits the confirmed `EnvironmentConfig.buckets` object (`buckets.{bucketId}.region` + `isCreated`). Region is a Tigris airport code (`iad` / `sjc` / `ams` / `sin`); AppHost `RailwayBucketRegion` maps those codes; unset defaults to `iad`. Do not send compute `Region.region` strings. Do not put region on `bucketCreate`. Canvas-created buckets already have an instance — adopt by name from `project.buckets` and skip the patch (region is immutable).
...
- Do not `serviceCreate` an image-less holder service per bucket. Credentials reach compute via `ConnectionStrings__{name}` at apply time. Leftover Offline services from earlier previews are not adopted or destroyed in v1.
