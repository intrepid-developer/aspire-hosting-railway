...
| `serviceInstanceUpdate` | Always pass `environmentId`. Fields used: `source.image`, `multiRegionConfig`, `sleepApplication`, `numReplicas` (only when `WithReplicas` is set alone — never with `multiRegionConfig`), `healthcheckPath`, `healthcheckTimeout`, `restartPolicyType`, `restartPolicyMaxRetries`, `startCommand`, `preDeployCommand`, `overlapSeconds`, `drainingSeconds`, `cronSchedule`. Official Postgres / Redis templates send `region` plus `numReplicas` 1 when a compute region is set; they do not send `multiRegionConfig`. A later update that omits `region` can reset the template to US West — re-send if set. Omit unset fields; do not send `null`. Do not add `vCPUs` / `memoryGB` or `registryCredentials` here. No GraphQL field named `serverless`. `ServiceInstance` has no `multiRegionConfig` read field. |
