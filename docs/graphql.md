...
| `serviceInstanceUpdate` | Always pass `environmentId`. Fields used: `source.image`, `multiRegionConfig`, `sleepApplication`, `numReplicas` (only when `WithReplicas` is set alone — never with `multiRegionConfig`), `healthcheckPath`, `healthcheckTimeout`, `restartPolicyType`, `restartPolicyMaxRetries`, `startCommand`, `preDeployCommand`, `overlapSeconds`, `drainingSeconds`, `cronSchedule`. Official Postgres / Redis templates send `region` plus `numReplicas` 1 when a compute region is set; they do not send `multiRegionConfig`. First-time template create is still `templateDeployV2` then at most one region update (`templateDeployV2` has no region field). A later update that omits `region` can reset the template to US West — include region on that same `serviceInstanceUpdate` when one is already being sent. Do not send a second region-only update in the same apply, and do not re-send region alone on later applies when it is already recorded. Omit unset fields; do not send `null`. Do not add `vCPUs` / `memoryGB` or `registryCredentials` here. No GraphQL field named `serverless`. `ServiceInstance` has no `multiRegionConfig` read field. |
...
| `serviceInstanceDeployV2` | Deploys a service instance from its current `source.image`. Official docs treat `serviceInstanceUpdate` as settings only; apply sends image + settings on Update, then this mutation once. Registry credential patches do not deploy. |
