...
Bucket `RailwayBucketRegion` codes (`iad` / `sjc` / `ams` / `sin`) are not compute `RailwayRegion` ids. Unset buckets stay `iad`. EU AppHosts must set `ams`. Bucket region cannot be changed after create (drop + recreate). Official Postgres / Redis `Region` is applied after the template service exists via `serviceInstanceUpdate`.
