...
Railway-specific settings use `PublishAsRailwayService`. Aspire.Hosting 13.5.3 has no `WithCpu` / `WithMemory` / healthcheck-timeout / restart-policy / start-command / overlap / drain / cron / custom-domain annotation. `WithArgs` is not mapped. Custom hostnames need `WithExternalHttpEndpoints()`.
